...
//
// The pod-template-hash is generated by Kubernetes and is typically 8-10 lowercase
// alphanumeric characters. We use a stricter check (8-10 chars) to avoid incorrectly
// stripping legitimate deployment name suffixes that happen to look like hashes.
// For example, a deployment named "my-app-v2beta" should not be parsed as "my-app"
// even though "v2beta" (6 chars) is alphanumeric.
func (r *PodWorkloadResolver) parseDeploymentFromReplicaSet(rsName string) string {
	...
	// Pod template hash is typically 8-10 lowercase alphanumeric characters.
	// Using a stricter range (8-10) to reduce false positives where deployment
	// names legitimately end with short alphanumeric segments.
