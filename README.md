...
Verify sidecars are injected by checking that pods show `2/2` (or `N+1/N+1`) in the READY column:
```bash
...
After configuring everything, verify L7 flow collection is working:

```bash
kubectl logs -l app.kubernetes.io/name=kestrel-operator -n kestrel-ai | grep "Purging L7"
```

You should see `num flows=<non-zero>` in the output, confirming Envoy access logs are being received.
