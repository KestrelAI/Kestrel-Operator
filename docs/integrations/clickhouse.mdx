...
- **Failed Mutations** — fires when the failed-mutation counter increases past a threshold within a poll window
...
- **Get / Update / Reset ClickHouse Setting** — tune server/query settings, e.g. `max_concurrent_queries` (Beta API; some settings trigger a rollout restart)
- **Get / Upsert / Delete Query Endpoint** — manage the query API endpoint (roles, allowed origins)
