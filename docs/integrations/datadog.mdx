...
Example: A workflow triggered by a Kubernetes incident queries Datadog for CPU and memory metrics around the incident timeframe, and if resource exhaustion is detected, creates a Datadog monitor to track the recovery.
