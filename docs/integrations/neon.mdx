---
title: "Neon"
description: "Connect Neon for serverless Postgres branch lifecycle, point-in-time restore, compute suspend/resume and autoscaling, reset-from-parent, credential rotation, and AI investigation in workflows"
---
...
- **Restore Branch (Point-in-Time)** — restore a branch to an earlier timestamp or LSN ("restore main to 2 hours ago"); the pre-restore state is preserved under a backup branch (gate behind an Approval node)
...
Example (recovery): After a bad migration or accidental data deletion, an Approval-gated workflow restores the branch to a timestamp just before the incident — the pre-restore state stays available under an auto-created backup branch.
