...
<CardGroup cols={2}>
  ...
  <Card title="Kyverno" icon="shield-check" href="/integrations/kyverno">
    **Triggers:** Policy Violation, Admission Blocked (auto-detected in-cluster, streamed from PolicyReports). **Actions:** List Policies, Get Policy, List Violations, Set Enforcement Mode, Apply Policy, Delete Policy.
  </Card>
  ...
  <Card title="Google Cloud" icon="google" href="/integrations/gcp">
    **Triggers:** Cost Anomaly, Budget Threshold, Cost Forecast Overrun, Daily Spend Spike, Idle Resource, Instance Preempted/Terminated Abnormally, Instance Group Unhealthy, GKE Node Pool Degraded, Cloud Run Revision Failed, Cloud Monitoring Alert, Log Error Pattern, Cloud Build Failed, Security Command Center Finding, Privileged IAM Grant, Service Account Key Aged, Public Storage Bucket, Public IAM Binding, BigQuery Job Failed, Expensive BigQuery Query, Cloud SQL Unhealthy, Pub/Sub Backlog, Dataflow Job Failed. **Actions:** Query Billing Data, Get Cost Anomalies/Forecast/Budget Status, Rightsizing and Committed Use Discount Recommendations, Find Idle Resources, Cost Remediation (Stop Instances, Delete Unattached Disks, Release Static IPs, Delete Old Snapshots, Downsize Instance), instance and node pool lifecycle, Cloud Run rollback and traffic split, Retry Cloud Build, Query Logs and Metrics, IAM and bucket hardening, BigQuery/Cloud SQL/Pub/Sub/Dataflow operations, Investigate (AI).
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Neon" icon="database" href="/integrations/neon">
    **Triggers (poll-based):** Branch Created, Branch Ready, Operation Failed, Compute Suspended, Compute Active, Usage/Quota Threshold. **Actions:** List/Get Project, List/Get/Create/Delete/Reset Branch, Restore Branch (Point-in-Time), List/Create/Delete Compute Endpoint, Suspend/Start Compute, Set Autoscaling Limits, Rotate Role Credentials, Get Connection URI, Investigate Neon.
  </Card>
...
<CardGroup cols={2}>
  <Card title="SonarCloud" icon="magnifying-glass-chart" href="/integrations/sonarcloud">
    **Triggers (webhook):** Analysis Completed (with quality gate result), Analysis Failed. **Actions:** Get Quality Gate Status, List Issues, List Security Hotspots, Get Project Metrics, Transition Issue, Assign Issue, Comment on Issue, Review Hotspot.
  </Card>
  <Card title="Okta" icon="id-badge" href="/integrations/okta">
    **Triggers (poll-based via the System Log):** User Locked Out, Suspicious Activity Reported, Admin Privilege Granted, MFA Factor Changed, User Created/Deactivated, Group Membership Changed, App Assignment Changed. **Actions:** Get User, List Users, Query System Log, List User's Groups, List Group Members, Suspend/Unsuspend User, Unlock User, Deactivate User, Clear Sessions, Expire Password, Reset MFA, Add/Remove User to/from Group.
  </Card>
</CardGroup>
...
GCP cost features work the same way through the [Google Cloud integration](/integrations/gcp). They additionally require a BigQuery billing export, since GCP has no general-purpose detailed cost API. Until the export is configured, cost blocks report that cost data is pending rather than failing.

- **Insights (read-only):** Query Billing Data, Get Cost Anomalies, Get Cost Forecast, Get Budget Status, Get Rightsizing Recommendations, Get Committed Use Discount Recommendations, Get Committed Use Discount Utilization, Compare Cost Periods, Find Idle Resources
- **Remediation (destructive — place behind an Approval block):** Stop Compute Instances, Delete Unattached Disks, Release Static IP Addresses, Delete Old Snapshots, Downsize Compute Instance
- **Cost triggers:** Cost Anomaly Detected, Budget Threshold Exceeded, Cost Forecast Overrun, Daily Spend Spike, Idle Resource Detected
