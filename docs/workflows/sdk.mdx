...
# Platform Requests (on-demand via Slack /kestrel-workflow, Kestrel Platform UI, or CLI)
Trigger.platform_request()  # Unified trigger — scope with filters
```

The platform request trigger accepts filter configuration:

```python
from kestrel.workflows import Trigger

# Accept all requests from any source/category
Trigger.platform_request()

# Scoped to Kubernetes create requests from Slack only
Trigger.platform_request().config(
    request_sources=["slack"],
    request_categories=["k8s"],
    request_types=["create"],
)

# Scoped to Cloudflare requests from any source
Trigger.platform_request().config(
    request_categories=["cloudflare"],
)
```
